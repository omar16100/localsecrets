...
/// `("a", "bc")` never produce the same bytes.
...
/// A 256-bit symmetric key. Zeroed on drop, never printed.
#[derive(Clone)]
...
impl DataKey {
    ...
    /// The raw key bytes. Only for wrapping and for handing to storage after
    /// encryption; never log or serialise these.
